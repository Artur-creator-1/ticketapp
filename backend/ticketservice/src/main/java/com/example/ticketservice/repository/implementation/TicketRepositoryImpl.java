package com.example.ticketservice.repository.implementation;
...
import static com.example.ticketservice.query.TicketQuery.*;
import static com.example.ticketservice.utils.QueryUtils.*;
import static com.example.ticketservice.utils.TicketUtils.randomUUID;
...
public class TicketRepositoryImpl implements TicketRepository {
    ...
    @Override
    public Task updateTask(String userUuid, String taskUuid, String name, String description, String status) {
        try {
            return jdbc.sql(UPDATE_TASK_FUNCTION)
                    .params(of("userUuid", userUuid, "taskUuid", taskUuid, "name", name, "description", description, "status", status))
                    .query(Task.class).single();
        } catch (EmptyResultDataAccessException exception) {
            log.error(exception.getMessage());
            throw new ApiException(String.format("No task found by UUID %s", taskUuid));
        } catch (Exception exception) {
            log.error(exception.getMessage());
            throw new ApiException("An error occurred. Please try again.");
        }
    ...
    public void deleteTask(String userUuid, String taskUuid) {
        try {
            jdbc.sql(DELETE_TASK_QUERY)
                    .params(of("userUuid", userUuid, "taskUuid", taskUuid))
                    .update();
        } catch (EmptyResultDataAccessException exception) {
            log.error(exception.getMessage());
            throw new ApiException(String.format("No task found by UUID %s", taskUuid));
    ...
    public User updateAssignee(String userUuid, String assigneeUuid, String ticketUuid) {
        try {
            return jdbc.sql(UPDATE_ASSIGNEE_FUNCTION).params(of("userUuid", assigneeUuid, "ticketUuid", ticketUuid)).query(User.class).single();
        } catch (EmptyResultDataAccessException exception) {
            log.error(exception.getMessage());
            throw new ApiException(String.format("No user found by UUID %s", assigneeUuid));
        } catch (Exception exception) {
            log.error(exception.getMessage());
            throw new ApiException("An error occurred. Please try again.");
        }
    ...
    public User getTicketUser(String ticketUuid) {
        try {
            return jdbc.sql(SELECT_TICKET_USER_QUERY).params(of("ticketUuid", ticketUuid)).query(User.class).single();
        } catch (EmptyResultDataAccessException exception) {
            log.error(exception.getMessage());
            throw new ApiException(String.format("No ticket found by UUID %s", ticketUuid));
        } catch (Exception exception) {
    ...
    private final BiFunction<Integer, Integer, Integer> offSet = (size, page) -> {
        var value = page == 0 ? 0 : page == 1 ? size : size * page;
        return value;
    };
}